...
def get_results(keyword, count, force=False):
    import pymongo
    ...
    if inserts != count:
    	if force == True:
    		...
    		e_insert_mismatch = 'You requested ' + str(count) + ' but only ' + str(inserts) + ' tweets are available in the ' + keyword + ' collection. Retry with force=True to ignore the mismatch.'
        ...
        print 'app/js/tweets.json has the latest ' + str(count) + ' tweets from the ' + keyword + ' collection in it.'
